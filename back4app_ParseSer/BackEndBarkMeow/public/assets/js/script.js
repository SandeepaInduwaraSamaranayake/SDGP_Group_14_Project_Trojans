...
{
  var seed = {"score":13371,"playerName":"Sean sPlott","cheatMode":false}
  this.xhttp.open("POST", Config.getUrl() + path, true);
  this.xhttp.setRequestHeader("X-Parse-Application-Id", $('#appId').val());    // here we are using JQuery to retrive appId from test.html script.
...
XHR.GET = function(path, callback)
{
  ...
  this.xhttp.setRequestHeader("X-Parse-Application-Id", $('#appId').val());    // here we are using JQuery to retrieve appId from test.html script.
  this.xhttp.setRequestHeader("Content-type", "application/json");
